...
class Cluster():
    ...
    def __init__(self, cur_total, size_a, size_b, support_matrix):
        self.cur_total = cur_total
        self.size_a = size_a
        self.size_b = size_b
        self.support_matrix = support_matrix
        self.dynamic_clustering(True)


    def dynamic_clustering(self, is_init, p=[0, 0], multip=100, radius=30, min_param_support=0.6, windowname='xxx'):
        if is_init:
            self.cur_total = 0
            self.size_a = self.size_b = int(3 * multip)
            ...
            cv2.imshow(windowname, (255 * self.support_matrix.copy() / self.support_matrix.max()).astype(np.uint8))
            cv2.waitKey(1)
            ...
            while True:
                ...
                if len(nearest_p) != 0:
                    ...
                    d_k = d_i + d_j
                    if d_k / self.cur_total >= min_param_support and d_k > 40:
                        # print('found max support')
                        return True
                    else:
...
def verify_baseline(baseline_vector, model_bifur_points, test_bifur_map, test_bifur_points,
    ...
    size_b = int(10)
    support_matrix = np.zeros(shape=(size_a, size_b)).astype(np.uint32)
    newcluster = Cluster(cur_total, size_a, size_b, support_matrix)
    ...
    while c_index < model_bifur_points.shape[0]:
        ...
        for s in similar_triangles:
            ...
            if newcluster.dynamic_clustering(False, a_b, min_param_support=min_param_support,
